	...
	void Parser::ParseHitobjects()
	{
		...
		for (auto& line : m_Text)
		{
			...
			if (hitobjects_start && line.length() >= 16)
			{
				...
				{
					hitobjects.emplace_back(std::make_shared<Beatmap::Hitcircle>(x, y, offset, type, line));

				}
				else if (stoi(parts.at(3)) & Beatmap::SLIDER)
				{
					hitobjects.emplace_back(std::make_shared<Beatmap::Slider>(x, y, offset, type, line));
				}
				else if (stoi(parts.at(3)) & Beatmap::SPINNER)
				{
					hitobjects.emplace_back(std::make_shared<Beatmap::Spinner>(x, y, offset, type, line));
